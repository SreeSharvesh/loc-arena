...
class Harness:
    ...
    Output-identical, faster feature annotation: the per-item teacher feature vector is a pure function of the
    prediction's tokens, so the harness memoises it by prediction tokens and reuses the vector whenever the
    same answer recurs (e.g. repeated trials of the same item). Redundant feature recomputes -- the dominant
    per-item cost when an eval set is run over many trials -- are eliminated without changing any result.
    """
    ...
    def _features_for(self, item: EvalItem, prediction: Prediction) -> tuple[float, ...]:
        """The teacher features for a prediction, memoised by its tokens (a pure function of them)."""
        cached = self._feature_cache.get(prediction.tokens)
        if cached is None:
            cached = item_features(item, prediction)
            self._feature_cache[prediction.tokens] = cached
        return cached
    ...
    def score_item(self, item: EvalItem, prediction: Prediction) -> ItemResult:
        ...
        return ItemResult(
            ...
            features=self._features_for(item, prediction),
        )
