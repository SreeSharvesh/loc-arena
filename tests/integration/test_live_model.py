...
from loc_arena.gateway.core import DirectTransport, GatewayCore, OpenRouterProvider
...
def test_live_model_turn_is_stamped_and_parses_a_tool_call(tmp_path: Path) -> None:
    ...
    edge = GatewayEdge(
        EP, DirectTransport(core), AppendOnlyLog(tmp_path / "mirror.jsonl", EP), clock=lambda: 100.0
    )
    ...
    brain = ModelBrain(
        client, "You are the lead engineer. Reply ONLY with a JSON tool call.", role="untrusted_agent"
    )
